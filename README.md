...
* Optional: the [coco-caption](https://github.com/tylin/coco-caption)
  library can be used for generating different evaluation metrics. To
  set it up, clone the repo into a local ```coco-caption``` folder and
  also make sure to run the ```get_stanford_models.sh``` script from
  within that repo.
...
In addition, in the paper we also compare the Object Relation Transformer to a model we call "Baseline Transformer", which does not use geometry in its attention (this is also denoted as "Up-Down + Transformer" in the table above). The comparison can be generated by running the commands in [neurips_report_comands.sh](neurips_report_comands.sh). The commands run the two models on the MSCOCO test set and generate the metrics for each models as well as a comparison of the models with respect to these metrics.
